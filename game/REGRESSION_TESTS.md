...
| Herald/archfiend warlock CRASH in late fights (agent-found 2026-06-26) | `updDemons` iterates `demons[]` by cached index, but `killEnemy()`'s infection-rise caps the horde via `demons.shift()` (front removal) reentrantly from inside that loop → cached `i` walks off the shrunken array → `demons[i]` undefined → `d.type`/`d.life` throws | one-line reentrancy guard `if(!d)continue;` in `updDemons` (skip empty slot) — **PUBLISHED build 1782514927 (RUN 2026-06-26 B)** | `demon-loop-survives-reentrant-horde-shift` ✅ |
| Wolf pack `wolves[]` unbounded-growth guard (agent-found 2026-06-27, defensive — no live bug) | the only bound on `wolves[]` is `updWolves`'s lifespan/hp cull; a slope-stress sweep confirmed it's a finite equilibrium, but no test pinned the cull → a future edit could silently remove it and grow the pack without bound in a long druid fight | no code change (cull already correct); added a PERMANENT perf case that PASSES live and FAILS on a cull-disabled mutant | `perf:wolves-bounded-by-lifespan-cull` ✅ |
