...
Death.finalize = function (world, ch, killerId, cause) {
  ...
  // Partner widowed
  const partner = ADV.World.byId(world, ch.partnerId);
  if (partner) { partner.partnerId = null; partner.exIds = partner.exIds || []; }
  ...
  if (killer) {
    ...
    if ((ch.exIds || []).includes(killer.id) || partner === killer) {
      killer.reputation -= 5;
