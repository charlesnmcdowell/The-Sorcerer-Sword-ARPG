...
Game.leaderPick = function (game) {
  ...
  // the guided first party quest stays at the lowest tier on the board
  if (game.tutorial && game.tutorial.step === 'partyQuest') return pool[0];
...
Game.startQuest = function (game, quest, opts) {
  ...
  game.quest.partnerAlong = roster.some(c => c.id === p.partnerId);
...
Game.startCombat = function (game, ambush) {
  ...
  const st = ADV.Combat.create(roster, q.enemies, {
    rng: game.rng.fork('combat' + game.world.questClock + ':' + q.encIdx),
