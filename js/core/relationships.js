...
Rel.tierBetween = function (world, fromId, toId) {
  ...
  if (a && a.partnerId === toId) return 'romantic'; // paired symmetric state (§6)
...
function enforceSlots(world, fromId) {
  ...
  const keep = new Set();
  if (ch && ch.partnerId) {
    const pe = mine.find(e => e.toId === ch.partnerId);
    if (pe) keep.add(pe);
  }
...
Rel.commit = function (world, aId, bId, feed) {
  ...
  for (const [x, y] of [[a, b], [b, a]]) {
    if (x.partnerId && x.partnerId !== y.id) {
      const ex = ADV.World.byId(world, x.partnerId);
      if (ex) {
  ...
  a.relationshipQuests = 0; b.relationshipQuests = 0;
  a.conceived = false; b.conceived = false;
  Rel.move(world, aId, bId, 100, 'romance', { set: true });
  Rel.move(world, bId, aId, 100, 'romance', { set: true });
  // Vault: the woman holds it; his assets transfer in (§7)
  const woman = a.sex === 'f' ? a : b;
  const man = a.sex === 'f' ? b : a;
  ADV.Vault.onCommit(world, woman, man);
  // Jealousy: women who wanted him turn on the wife, scaled by his wealth/rank (§6)
...
Rel.jilt = function (world, leaver, abandoned) {
  const lines = [];
  leaver.partnerId = null; abandoned.partnerId = null;
...
Rel.socialRate = function (world, fromCh, toCh) {
  ...
  if (toCh.sex === 'm') {
    if (toCh.partnerId) mult *= 1.3;                       // partnered men befriend everyone faster
