...
// Full battle roster including followers (conscripts/undead), capped by field rules.
Party.battleRoster = function (world, leadCh) {
  ...
  if (p && p.leaderId === leadCh.id) {
    for (const m of Party.members(world, p)) { out.push(m); addFollowers(m); }
  } else if (p) {
    // hireling: bring the whole employer party
    const l = Party.leader(world, p);
    if (l && l.alive && l !== leadCh) { out.push(l); addFollowers(l); }
    for (const m of Party.members(world, p)) if (m !== leadCh) { out.push(m); addFollowers(m); }
  }
  return out;
};
