...
**Not done:** the 20 new personalities and the 16 campaign2 characters have no
ElevenLabs voice ids cast, so their lines display as text with no audio (the
audio layer treats a missing clip as a silent line). `ADV.DATA.DIALOGUE2_UNCAST`
lists them; add ids to `tools/voice_casting.json` and run `tools/gen_voices.py`.
