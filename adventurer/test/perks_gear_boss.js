...
console.log('\n-- Perks start at level 1 and level like actives --');
{
  ...
  eq(ADV.SkillSys.manifest(t, t.perks[0]).data.reflectPct, 0.35, 'L10 Bulwark reflects 35%');
  t.perks[0].level = 25;
  eq(ADV.SkillSys.manifest(t, t.perks[0]).tier, 'advanced', 'L25 is advanced');
  eq(ADV.SkillSys.manifest(t, t.perks[0]).data.reflectPct, 0.5, 'L25 Bulwark reflects 50%');
...
console.log('\n-- Gods take three turns and add 50% max-HP damage --');
{
  ...
  const hit = st.events.find(e => e.t === 'damage' && e.uid === uv.uid);
  ok(hit && hit.dmg >= Math.round(uv.maxHp * ADV.DATA.CONST.GOD_HIT_PCT), 'god damage includes a share of target max HP (GOD_HIT_PCT)', hit && hit.dmg);
