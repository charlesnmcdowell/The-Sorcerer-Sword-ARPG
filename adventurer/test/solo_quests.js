...
{
  const { g } = gameOf(0);
  const s300 = g.board.filter(q => q.track === 'solo' && q.payout === 300);
  const s600 = g.board.filter(q => q.track === 'solo' && q.payout === 600);
  ok(s300.length === 1, 'one 300g solo bounty is posted');
  ok(s600.length === 1, 'one 600g solo bounty is posted');
...
console.log('-- 600g finale uses mini-boss setup rules --');
{
  ...
  ok(bosses.length >= 1 && bosses[0].hpFloor >= ADV.Character.maxHp(p), 'mini-boss HP floors at the player');
