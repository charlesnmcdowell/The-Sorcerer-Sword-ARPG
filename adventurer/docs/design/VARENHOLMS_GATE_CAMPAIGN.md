...
The campaign has its own score (`audio/music/vg_*.mp3`, ElevenLabs Music, 2:00 loops,
generated by `tools/gen_c3_music.py`, prompts and approvals in `tools/c3_music.json`).
`D.CAMPAIGN3_MUSIC` maps it; `C3.musicFor(quest)` hands `Music.startRun` a scored run.

- One underscore per quest (Lanternhold, the Shore Road, Thornbury, the mines, the
  Gnashing Wood, the Mirkhollow, the iron mine, the Gate, the Consortium tower, the
  catacombs, the hunted city, the coronation, the Undercity, the temple). It plays on
  every screen and under every line of the quest; it never rotates.
- Battle cues are heard only inside the Combat scene: `vg_battle` for Quests 1–7,
  `vg_battle_city` for 8–14. The underscore pauses for the fight and resumes where it
  left off. Mid-campaign bosses and minibosses use the battle cue; `vg_boss` plays only for the
  named antagonist's fight (`isBossFight`), i.e. Kolade at the altar.
- Cues laid over a scene: `vg_dream` for any beat with `dream:true` (and the replies to
  its choice), cleared by the first waking beat; `vg_ending` under the epilogue card.
- The hub plays `vg_camp` instead of the home theme while the campaign is under way
  (`C3.hubMusic`, `Music.homeOverride`); the home theme returns once it is finished.
- Levels: underscores are normalised to −18 LUFS, battle cues to −14, the boss to −13,
  and the music channel ducks to 45 % while any voice clip plays (`Music.duck`), so
  dialogue is never under the score. `test/music_score.js` covers all of it.
- No stock personality voices anywhere in the campaign: `C3.spawnEnemy` / `spawnEncounter` strip
  `personalityId` from every enemy and mini-boss, so the combat taunts that would have played in a
  random stock voice stay silent; only cast characters with designed voices speak.
