...
(async()=>{
 ...
 assert.equal(Object.keys(state.completed).length,patch.entries.length,'All reviewed recordings must finish before installation');
 ...
 try{
  ...
  for(const e of patch.entries){
   ...
   const words=e.shown.trim().split(/\s+/).length;
   assert.ok(sound.duration>=words/5.5,'Implausibly short recording: '+e.path);
   checks.push({path:e.path,voice:e.voice,audioHash:rec.audioHash,words,wordsPerSecond:words/sound.duration,...sound});
  }
 }finally{await browser.close();}
 const backup=path.resolve('archive/dialogue/2026-09-16-gate-epilogue');
 ...
 for(const e of patch.entries){
  ...
  const slot=manifest.entries.findIndex(x=>x.path===e.path),entry={...e};delete entry.before;
 ...
 globalState.externalPatchRuns??=[];const id='20260916-epilogue-v2';
 ...
 for(const file of ['data/campaign3_data','data/campaign3_dialogue','data/gate_finale','data/gate_epilogue_voice','data/voice_manifest','core/campaign3','core/gate_finale','ui/campaign3_ui']){
  html=html.replace(new RegExp('(js/'+file+'\\.js)(?:\\?v=[^"\\s]+)?'),'$1?v='+id);
 }
 ...
 console.log(JSON.stringify({installed:checks.length,billed:state.reportedClipCosts,seconds:checks.reduce((sum,c)=>sum+c.duration,0),bytes:patch.entries.reduce((sum,e)=>sum+state.completed[e.hash].bytes,0)},null,2));
